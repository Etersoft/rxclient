    ...
    if (cfg.IsValid()) {
        ...
        if (m_pCfg->bGetUseSmartCard()) {
            ...
            if (m_pCfg->sGetSshKey().IsEmpty()) {
                ...
                nxsshcmd << wxT(" -i ") << cygPath(fn.GetShortPath());
            }
        }
        ...
        wxSetEnv(wxT("NX_HOME"), cygPath(fn.GetShortPath()));
        wxLogInfo(wxT("env: NX_HOME='%s'"), cygPath(fn.GetShortPath()).c_str());
        wxSetEnv(wxT("HOME"), cygPath(fn.GetFullPath()));
        wxLogInfo(wxT("env: HOME='%s'"), cygPath(fn.GetShortPath()).c_str());
        ...
        m_pDlg = &dlg;
        dlg.Show(!wxGetApp().NoProgressBar());
